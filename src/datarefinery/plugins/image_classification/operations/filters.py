...
def filter_by_label(
    ...
    *,
    ...
    if label_field is None:
        raise PluginError(
            "filter_by_label requires Labels.field to be declared"
        )
    labels_param = params.get("labels")
    if not isinstance(labels_param, (list, tuple)):
        raise PluginError(
            f"filter_by_label 'labels' must be a list/tuple "
            f"(got {type(labels_param).__name__})"
        )
    ...
    raise PluginError(
        f"filter_by_label 'action' must be 'include' or 'exclude' "
        f"(got {action!r})"
    )
...
def random_sample(
    ...
    *,
    ...
    if not isinstance(seed, int):
        raise PluginError(
            f"random_sample requires integer 'seed' "
    ...
    if (fraction is None) == (n_param is None):
        raise PluginError(
            "random_sample requires exactly one of 'fraction' or 'n'"
        )
    n_records = len(records)
    if fraction is not None:
        if not 0.0 <= float(fraction) <= 1.0:
            raise PluginError(
                f"random_sample 'fraction' must be in [0, 1] "
                f"(got {fraction!r})"
            )
        ...
        if target < 0:
            raise PluginError(
                f"random_sample 'n' must be non-negative (got {n_param!r})"
            )
