...
"""Sink format writers (Story I.d).

v1 ships :func:`write_png_per_record`, which expects a uint8 HxWxC (or
HxW) numpy array on the named record field and writes a single PNG via
``PIL.Image.fromarray``. Additional formats (`npy_per_record`,
`parquet`, `tar`) are listed in the spec § 3.4 and deferred to Future.
"""
