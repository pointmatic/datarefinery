...
"""Plugin contract tests for the text plugin stub (Story C.c).

The text plugin ships in v1 as a stub: schemas declared so recipes
...
from datarefinery.plugins.text import PLUGIN
...
def test_operation_factory_raises_plugin_error() -> None:
    with pytest.raises(PluginError, match="stub plugin; not implemented"):
        PLUGIN.operation_factory("Transformations", "lowercase")


def test_discover_plugins_returns_text() -> None:
    plugins = discover_plugins()
    assert "text" in plugins
    assert plugins["text"].is_stub() is True
