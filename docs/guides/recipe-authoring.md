...
Common templates: `{label}/{file}` (= bare `image_folder`), `**/{label}/{file}` (label-at-any-depth), `{split}/*/{label}/{file}`, and `**/{file}` (flat-at-any-depth; pair with `label_from` for sidecar labels or `unlabeled: true`). A `*_tree` source with a `{label}` token must **not** also set `label_from` (two label sources); a `*_tree` with no `{label}` takes labels from `label_from` or is `unlabeled`. **Validator check 31** enforces this statically: a labeled `*_tree` source with no label source (no `{label}`, no `label_from`, not `unlabeled`), or with two (both `{label}` and `label_from`), fails fast at `validate` naming the source — instead of failing deep in `materialize`. `record_id` is the file's path relative to the source root, prefixed by the source name. The `layout` field is **additive to cache identity** — recipes that don't use a `*_tree` source are byte-identical (unchanged `recipe_hash`); the bare `image_folder` / `image_flat` types are unchanged. The shared `path_tree` resolver ([`recipe/layout.py`](../../src/datarefinery/recipe/layout.py)) walks the tree via the symlink-following enumeration (Story K.g), so loader and hasher agree on the file set. (`validate` is filesystem-blind, so a layout whose `{label}` level holds only subdirectories is still caught at `materialize` with a "matched no files" error.)
