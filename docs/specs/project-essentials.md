...
### The loader and the input hasher must walk the *same* file set (one shared enumeration)

Input ingestion has **two** independent file walks: the **loader** (which reads the actual records) and the **input hasher** (`_iter_files`, which digests the raw input bytes for cache identity). These two MUST enumerate the identical file set. When they diverge, the cache key is computed over a different set of bytes than the loader actually reads — the silent stale-cache / wrong-data class of reproducibility bug. This is exactly the Gap 2 defect (Phase K, Story K.g): on Python 3.12 the hasher's `root.rglob("*")` does not descend symlinked directories (`recurse_symlinks` is 3.13+), so a symlinked-dir tree hashed to an effectively empty set while the loader read the real files — two different symlink views collided on one hash. As of Phase K Subphase K-2, the loader and hasher share **one enumeration helper**, and the generalized `path_tree` resolver (Story K.h, the `image_tree`/`audio_tree` source types) is the single cross-plugin point where "which files belong to this source" is decided — the image and audio plugins both delegate to it rather than reimplementing per-modality walks.

**Why:** the reproducibility guarantee is byte-identical re-runs over the *resolved* input set. If the hasher and loader can disagree about which files exist (symlinks, recursion depth, extension filtering, ordering), the cache identity stops being a faithful fingerprint of the bytes consumed — and the divergence is invisible until someone gets a stale hit on changed data. Unifying enumeration removes the entire class.

**How to apply:** when touching `pipeline/inputs.py` (the `path_tree` resolver, `_iter_files`, `_hash_image_folder`/`_hash_image_flat`, the `_load_one_*` loaders) or the audio plugin's `plugins/audio_classification/inputs.py`, never introduce a second, parallel file walk for hashing or loading. Route both through the shared enumeration helper. Traversal must stay **deterministically sorted** and must **follow symlinked directories with cycle protection** (dedupe on resolved real-paths). Tempting move to refuse: "the hasher only needs file bytes, so a quick separate `glob` here is fine." **No** — that is precisely how the loader and hasher drifted apart in Gap 2; a divergent walk is a reproducibility regression even if it looks locally correct.

### `feature_path` is instance-root-relative (the sink-output bucket), NOT `dataset/`-relative

The per-record **`feature_path`** field (the audio float-feature-array egress surface, Phase K Subphase K-1, `npy_per_record` sink) resolves **relative to the instance root** — `<instance>/<feature_path>` (e.g. `<instance>/features/<split>/<record_id>.npy`). This is the **same bucket as the J.g sink-rewritten `path`** (sink output lands at `<instance>/...`, a sibling of `dataset/`). It is **NOT** the `image_path` bucket, which is `dataset/`-relative. The two anchors genuinely differ, and conflating them is the documented silent-misalignment trap: a consumer that joins `feature_path` onto `dataset/` resolves to a path that does not exist (or, worse, a wrong file). This is the ratified contract (DR ↔ MF 2026-06-23 review round, vendor-spec Q1) — see [`modelfoundry/vendor-dependency-spec.md` § "Audio feature-array persistence"](modelfoundry/vendor-dependency-spec.md).

**Why:** `feature_path` is a **shape-binding surface** (see § "Recipe / manifest / report shape changes need a cross-repo coordination check") — ModelFoundry builds its feature-array loader branch against this anchor. An earlier draft of the consumer's MF gap doc described it as "relative to `dataset/`, mirrors `image_path`," which is the wrong analogy; sink output follows the sink-`path` anchor, not the `image_path` anchor. A producer/consumer disagreement on the anchor is invisible at `validate`/`materialize` and only surfaces as a `FileNotFoundError` (or silent wrong-file load) deep in a training run.

**How to apply:** when implementing or touching the `npy_per_record` sink (`pipeline/sinks/`), the `feature_path` rewrite at dataset serialization, or any doc that describes it, anchor `feature_path` exactly as the J.g sink-rewritten `path` (instance-root-relative), never as `image_path` (`dataset/`-relative). Treat it as a relative POSIX path joined onto the instance root verbatim — it may be **nested** (window `record_id` can carry `/`) and is **authoritative over any stray source `path`** on the same record. Persist the **pre-normalize `mel`** field (not the normalized `feature`), or the consumer double-normalizes when it applies the persisted `audio_normalize` stats at load.
