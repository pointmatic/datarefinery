...
### Story I.v: G7 — Stage-aware visualization dispatch (closes G5) [Done]
...
Closing G5: `augmented_sample_grid` runs at `stage: post_Filters` and reads uint8 records by construction (the `pre_transformations` literal in the original story-body text is the *intent*; the closed `VizStage` vocabulary uses `post_<Stage>` form per the I.v scope-decision Q&A).

**Tasks:**

- [x] Constrained `VisualizationOp.stage` to a closed `VizStage` Literal in [`recipe/models.py`](../../src/datarefinery/recipe/models.py): `{post_InputContracts, post_Filters, post_Splits, post_Generation, post_Transformations, post_Augmentations, post_Featurizations, post_pipeline}`. Mirrors `SinkStage`'s grammar but drops the entries that don't change records (post_OutputExpectations / post_Visualizations would be functional duplicates) and keeps `post_pipeline` as the alias for the final snapshot (the scaffolder default). Unknown-stage typos now fail at model-validate time.
- [x] Extended validator **check 11** (renamed `visualization_mode_declared` → `visualization_well_formed`; no new check number — kept stable to avoid churning the integration `N/N checks passed` assertions): rejects a viz whose declared stage's recipe section is empty (e.g., `stage: post_Generation` with no `Generation:` ops). Stages that always exist (`post_InputContracts`, `post_Filters`, `post_Splits`, `post_pipeline`) pass regardless of section content.
- [x] Extended [`pipeline/runner.py`](../../src/datarefinery/pipeline/runner.py) with a `viz_snapshots: dict[str, Mapping[str, list[Record]]]` dictionary populated at the END of each named runner stage — shallow copies of `split_map` keyed by the `VizStage` literal. `post_pipeline` is snapshotted just before `apply_reporting_visualizations` runs. `post_InputContracts` wraps the pre-Splits flat record stream under `{"_records": [...]}` (matching the Sinks convention).
- [x] Changed `apply_reporting_visualizations` in [`pipeline/stages/visualizations.py`](../../src/datarefinery/pipeline/stages/visualizations.py) to accept either a snapshots map (`{stage: {split: list[Record]}}`) or a flat splits dict (auto-wrapped as the lone `post_pipeline` snapshot via a small `_normalize_stage_snapshots` helper). Each viz op dispatches against `snapshots[op.stage]`; a missing snapshot raises `MaterializeError` listing what *is* present (important for the re-render path, which only has `post_pipeline`). The backward-compat wrapping kept all 25+ pre-existing viz test callsites green without rewrites.
- [x] Scaffolder default is unchanged — already emits `stage: post_pipeline`, which is now a valid `VizStage` literal.
- [x] Integration coverage: the two new tests in [`test_visualizations_augmented_sample_grid.py`](../../tests/plugins/image_classification/test_visualizations_augmented_sample_grid.py) (`test_g5_close_out_augmented_sample_grid_runs_at_post_filters` and `test_g5_close_out_same_viz_at_post_transformations_still_fails`) drive the stage-aware dispatch end-to-end through `apply_reporting_visualizations` against both a uint8 `post_Filters` snapshot and a float `post_Transformations` snapshot, asserting the PNG is written when the viz targets the uint8 snapshot and that the same viz crashes when pointed at the float snapshot — pinning that *stage selection* is what makes the fix work.
- [x] G5 close-out test landed (see the test pair above).
- [x] Kept the `_tile` clip-cast at [`augmented_sample_grid.py:144-145`](../../src/datarefinery/plugins/image_classification/visualizations/augmented_sample_grid.py#L144-L145) as **defense-in-depth and documented** rather than removing it: the canonical placement (`post_Filters`) is uint8 by construction, but an author can legitimately point the viz at a later stage (e.g. `post_Featurizations` after a cast). The clip prevents a hard PIL `TypeError` in that non-canonical-but-valid case.
- [x] DOC: added "Stage-aware dispatch (G7)" subsection to [`recipe-authoring.md` § Visualizations](../guides/recipe-authoring.md) — vocabulary table, pre-vs-post-normalize worked example, validator empty-stage rule, re-render limitation.
- [x] Updated [`phase-i-dependency-gaps-v0.16.0.md` § G5 + § G7](phase-i-dependency-gaps-v0.16.0.md): status blocks (G7 closed; G5 closed via G7), priority summary rows, workarounds rows.
- [x] Cross-repo coordination: extended the `report/visualizations/<viz_name>.png` bullet in [`dependency-spec.md`](modelfoundry/dependency-spec.md) to note that stage-aware dispatch is **internal** — the on-disk surface (flat PNGs per viz op; single `report.md` section) is unchanged. Per-stage report subsections remain in `stories.md § Future`.
- [x] Migrated five stale `stage: "post_split"` viz-test placeholders in [`test_validator.py`](../../tests/unit/test_validator.py) to `"post_pipeline"` (the previous placeholder string wasn't in any vocabulary; the new `VizStage` Literal rejects it).
- [x] CI parity: `pyve test` 1215 passed (+7 from this story: 4 new model + check-11 tests, 2 G5 close-out, 1 re-purposed); `pyve testenv run mypy src tests` clean across 196 source files; `pyve testenv run ruff check src/ tests/` clean; `pyve testenv run ruff format --check src/ tests/` clean.
...
### Story I.z: Phase J planning context prompt (ModelFoundry + NbFoundry integration) [Done]
...
- [x] Verified the prompt's accuracy against the post-I.y baseline (Phase I closed, v0.19.0 shipped, schema_version 2 canonical, Bundle 4 reshape cluster `[Done]`). The prompt's "verify current repo state" instruction at line 34 plus its historical parenthetical (intentionally left in place as a "snapshots lag reality — re-verify" reminder) cover any future drift. Added a "Last refreshed: 2026-05-30 (Story I.y close)" stamp near the top so the next planner knows when the gap list was last checked. No further content edits needed.
- [x] No code, no test change, no version bump. Bundled with no release (documentation handoff; the developer runs `plan_phase` in a fresh conversation when ready to start Phase J).
