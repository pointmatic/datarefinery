...
### Story G.c: v0.6.4 Release Automation Polish [Done]

- [x] Add a GitHub Action that on tag push extracts the corresponding `CHANGELOG.md` section and creates a GitHub Release with that body.
- [ ] Add tag protection rule: only maintainers can push `v*` tags. *(Developer action: GitHub UI → Settings → Tags → Tag protection rules. Documented in `docs/guides/releasing.md`; not expressible in workflow YAML.)*
- [x] Document the release procedure in `docs/guides/releasing.md` (bump → CHANGELOG → tag → workflow → verify).
- [x] Bump version to v0.6.4
- [x] Update CHANGELOG.md
- [ ] Verify: a new tag push produces a GitHub Release with the changelog body ~~and a successful PyPI upload~~. *(Developer action: requires push of a `v*` tag. PyPI upload is deferred — the `datarefinery` name is taken on PyPI; the GitHub Release is the v1-era release artifact.)*
