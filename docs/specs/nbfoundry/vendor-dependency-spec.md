...
**Schema v2 → v3 (v0.22.0, Phase J Recipe Architecture bundle) — segmented recipe identity + per-segment versioning.** Cache identity moved from the flat canonical sha256 to a **segmented** hash; this is a **one-time pre-1.0 cache invalidation** (every instance re-materializes once). The recipe shape on disk is **unchanged** (segmentation is internal — the v2→v3 bootstrap stamps the version only, no field reshape), so NbFoundry — which drives the library/CLI rather than reading recipe internals — needs no binding changes beyond widening its tracked support set to include `3`. Three further bundle changes are likewise binding-neutral for NbF and only matter to cells that *read* recipe internals:

- **Per-segment versioning (Story J.n.7).** Each identity segment (`core`, `plugin:image`, `plugin:audio`, `overlays`, `extensions`) evolves on its own version axis; there is no global umbrella counter. The flat `recipe.schema_version` stays the on-disk era marker and the consumer-facing coordination counter (no on-disk segment-version block), so NbF's obligation is unchanged: track the flat `SUPPORTED_SCHEMA_VERSIONS` set.
- **`extensions` namespace (Story J.n.6).** A new optional top-level `extensions: {<namespace>: {<key>: <value>}}` block for experimental, plugin-consumed parameters; additive (empty ⇒ no cache change). NbF cells generally ignore it.
- **No implicit defaults (Story J.n.4).** Op `params` in `recipe.json` now contain exactly what the author/scaffolder wrote — no code-supplied default layer. A doc-rendering cell reading op params sees only authored values.

Full detail (segmented algorithm, per-segment versioning + migration registry, `AudioSource` union, extensions, no-implicit-defaults) is in the MF spec § Cache-identity contract and § Segment-scoped recipe shape.
