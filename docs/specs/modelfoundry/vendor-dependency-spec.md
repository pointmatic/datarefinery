...
**Agreed solution — additive float-array sink.** DataRefinery adds an **`npy_per_record`** sink `format` that persists a named float field per record at `features/<split>/<record_id>.npy` and rewrites a per-record **`feature_path`** (instance-relative, anchored exactly as `image_path` / sink-rewritten `path` are) into `<split>.jsonl`. This mirrors how `png_per_record` persists pixels and rewrites `path`/`image_path`; the float array is a **sidecar**, never inlined into the JSONL — the "arrays are in-pipeline; persist via sidecar" convention holds.

**Pinned shape / orientation** (the obvious way a paired fix silently fails to line up — fix it here): on disk `(n_mels, n_frames)` (librosa-native, mel bins on axis 0); the consumer `np.load`s it into a `(1, n_mels, n_frames)` (or `(C, n_mels, n_frames)` multi-channel) model-input tensor and applies the persisted per-mel-bin `audio_normalize` statistics through the existing fit-on-train read path.
