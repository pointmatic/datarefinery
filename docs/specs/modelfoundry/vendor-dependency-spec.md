...
>
> **DR: 2026-06-23 — `npy_per_record` producer side SHIPPED (Story K.c, v0.24.0).** The § "Audio feature-array persistence" is re-ratified **forward-declared → shipped on the DataRefinery side**: the additive `npy_per_record` sink writer (`write_npy_per_record`, `float32`) + the instance-root-relative `feature_path` rewrite (`feature_path_rewrite_plan` → `_prepare_record_for_persistence`) + manifest wiring (`manifest.sinks[<name>].format == "npy_per_record"`) are real. Additive, **no recipe `schema_version` bump** (new `SinkOp.format` enum value + opt-in optional `feature_path`); sink output is covered by `(recipe_hash, input_hash, seed)` cache identity (byte-identical `.npy` across runs). The **consumer half remains pending MF `plan_features`** (the feature-array loader branch + the `audio_normalize`-at-load path) — neither half unblocks the consumer alone. Also: the paired briefs were relocated per the DR doc-layout decision — DR brief → `DR:docs/specs/audio-feature-persistence.md`, MF brief mirror → `DR:docs/specs/modelfoundry/audio-feature-consumption.md`; stale relative links in this doc updated.
...
#### Audio feature-array persistence — `npy_per_record` + `feature_path` (DR producer side shipped, v0.24.0)

> **Status: DataRefinery producer side SHIPPED (Story K.c, v0.24.0); ModelFoundry consumer loader pending `plan_features`.** This pins the agreed cross-repo shape so a downstream model can consume prepared audio features. The paired rollout: DataRefinery shipped the additive `npy_per_record` sink + the `feature_path` rewrite (Story K.c — the producer half is now real); ModelFoundry ships the feature-array loader branch (via `plan_features`). **Neither half unblocks the consumer alone; the consumer remains blocked until MF's loader lands.** Sources: DR [`consumer-gap-solutions.md`](../consumer-gap-solutions.md) Gap 3 (`DR:docs/specs/consumer-gap-solutions.md`), MF [`modelfoundry/consumer-gap-solutions.md`](consumer-gap-solutions.md) Gap 3 (`MF:docs/specs/consumer-gap-solutions.md`), and the paired briefs [`audio-feature-persistence.md`](../audio-feature-persistence.md) (`DR:docs/specs/audio-feature-persistence.md`) / [`audio-feature-consumption.md`](audio-feature-consumption.md) (`MF:docs/specs/modelfoundry-audio-feature-consumption.md`).
...
**Agreed solution — additive float-array sink (shipped, Story K.c).** DataRefinery added an **`npy_per_record`** sink `format` that persists a named float field per record at `features/<split>/<record_id>.npy` (`float32`) and rewrites a per-record **`feature_path`** into `<split>.jsonl`. This mirrors how `png_per_record` persists pixels and rewrites the JSONL `path`; the float array is a **sidecar**, never inlined into the JSONL — the "arrays are in-pipeline; persist via sidecar" convention holds. Implementation: `DR:src/datarefinery/pipeline/sinks/writers.py::write_npy_per_record` (writer), `DR:src/datarefinery/pipeline/path_rewrite.py::feature_path_rewrite_plan` + `DR:src/datarefinery/pipeline/runner.py::_prepare_record_for_persistence` (the instance-root-relative `feature_path` rewrite).
...
**Versioning.** Additive — a new `SinkOp.format` enum value plus a new optional per-record `feature_path` field; existing recipes' canonical bytes are unchanged and the format is opt-in, so **no recipe `schema_version` bump**. (This supersedes the earlier "a float on-disk format would require a `schema_version` bump" note under § "Normalization is applied by the consumer".) Now shipped (v0.24.0): `manifest.sinks[<name>].format` reports `npy_per_record`, and the `feature_path` field + the on-disk `features/<split>/` tree are **bound shape surfaces** on the producer side. (The consumer half — MF's feature-array loader branch — remains pending `plan_features`.)
...
**`*_tree` source types + `layout` template (Story K.h, v0.25.0 — additive).** A new source-type family expresses dataset directory structure as a path-template `layout` instead of an enumerated flavor: `type: image_tree` / `audio_tree` with `layout: str` (components `{label}` / `{split}` / `{file}` + wildcards `*` / `**`). This resolves arbitrary-depth taxonomy trees (`category/class/file`, `split/category/class/file`) that the one-level `image_folder` / `audio_folder` could not. The existing `image_folder` / `image_flat` / `audio_folder` / `audio_flat` types are **unchanged** (no migration; byte-identical ordering and `record_id`s preserved). **Cache identity:** the new `InputSource.layout` field is **additive** — it is excluded from canonical bytes when absent (`None`), so every recipe that does not declare a `*_tree` source keeps a **byte-identical `recipe_hash`** (no invalidation; the segment pins stay green). When present, the `layout` text enters the `core` segment's canonical bytes. `record_id` for a `*_tree` record is `f"{source}/{rel_posix}"` (the file's path relative to the source root) — the same scheme image recipes already use; a `{split}` token stamps the record's `partition` (mutually exclusive with a per-source `partition`).
...
- `MF:docs/specs/modelfoundry-audio-feature-consumption.md` — the MF consumption brief (paired with `DR:docs/specs/audio-feature-persistence.md`).
