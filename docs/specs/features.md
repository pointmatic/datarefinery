...
**Status (Story H.r.1, removed in H.r.2):** aggressive-mode is wired into `pipeline.runner` and unit-tested, but **persistence of augmented image bytes is not yet implemented** (`_write_dataset._coerce` drops numpy `image` arrays — see the H.o spike findings). A user declaring `materialization: aggressive` in a real recipe hits a runner-level `MaterializeError("…pending Story H.r.2…")` at materialize time, and `validate` raises a matching warning (check 23). The runner guard and validator warning both go away in Story H.r.2 once sidecar PNG persistence lands.
